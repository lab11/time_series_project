...
import numpy as np
import sys
from plaid_data_setup import train_phased_cycle_nn
...
def gen_data():
    ...
    for i in range(TrainingData.shape[0]):
        TrainTimeTraces.append(time_trace)

    ValidTimeTraces = []
    for i in range(ValidationData.shape[0]):
        ValidTimeTraces.append(time_trace)

    TrainTimeTraces = np.array(TrainTimeTraces)
    ValidTimeTraces = np.array(ValidTimeTraces)

    TrainTimeTraces = np.reshape(TrainTimeTraces, (TrainTimeTraces.shape[0], TrainTimeTraces.shape[1], 1))
    ValidTimeTraces = np.reshape(ValidTimeTraces, (ValidTimeTraces.shape[0], ValidTimeTraces.shape[1], 1))

    return (TrainingData, ValidationData, TrainingLabels, ValidationLabels, TrainingNames, ValidationNames, labelstrs, num_names, TrainTimeTraces, ValidTimeTraces)
...
train_phased_cycle_nn(lstm_graph, data_input, correct_labels, time_input, train_optimizer, None, evaluation_args, gen_data())
