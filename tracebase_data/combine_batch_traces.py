...
class_map = []
name_map = []
labels = np.ndarray(len(fnames))
dev_ids= np.ndarray(len(fnames))
...
for fname in sorted(fnames):
    ...
    labels[index] = name_map.index(device_name)
    dev_ids[index] = class_map.index(class_name)
...
np.save(dirname + "trace_classes", labels)
np.save(dirname + "trace_dev_ids", dev_ids)
